...
<div class="theorem">
**Geometric Version of Heine-Borel.** Let $V$ be a real vector space and $A \subseteq V$ be a convex linearly open set. Then if $F$ is an affine set $F \cap A =\varnothing$, there exists an affine hyperplane $H$ such that $H \cap A = \varnothing$, $H \supseteq F$, $x \in H$.
</div>

<div class="theorem">
**Theorem.** If $V$ is a topological vector space over $K = \R,\mathbb{C}$, $A$ convex and open which are nonempty, $F$ is an affine subspace such that $F \cap A = \varnothing$, then $\exists H$ a closed affine hyperspace $F \subseteq H$ and $H \cap A = \varnothing$.
</div>
...
<div class="theorem">
**Theorem.** If $A \subseteq V$, $V$ is locally convex topological vector space, $A$ is a closed convex set, $x \not\in A$. Then there exists $f : V \rightarrow K$, continuous? and linear such that 
<center>$$\inf_{y\in A} |f(x) - f(y)| > 0$$</center>
(in particular, the affine hyperplane $H = \{y : f(x) = f(y)\}$ satisfies $H \cap A = \varnothing$).
</div>

As a corollary,
<div class="theorem">
**Corollary.** $W \subseteq V$ linear subspace, $x \in \overline{W} \Leftrightarrow \forall f : V \rightarrow K$ continuous, $f |_W = 0$, we have $f(x) = 0$. 
</div>

{/* Hahn Banach theorem was actually proved by Helli? earlier, and was proved int he real case first historically. */}

<div class="theorem">
**Theorem.** IF $W \subseteq V$, $p$ is a seminorm of $V$, $f : W \rightarrow K$, $|f(x)| \le p(x)$, then $\exists f_1 : V \rightarrow K$ such that $|f_1(x)| \le p(x)$ and $f_1|_W = f$.
</div>
...
<div class="theorem">
**Theorem (Riesz).** Suppose $L : H \rightarrow K$, linear and continuous, then $\exists !$ $y \in H$ such that $L(x) = (x,y)$. 
</div>
...
<div class="theorem">
**Theorem (Application of Hein Borel and $\R$).** For $\varphi \in C^2(\mathbb{C})$, suppose $\varphi : \mathbb{C} \rightarrow \R$ such that $\Delta \varphi(z) = \kappa(z) > 0$ for all $z \in \mathbb{C}$. Then for all $f \in L^2 (\mathbb{C}, \kappa^{-1}ee^{-\varphi}\; dm(z))$, ($\|f\|_\psi^2 := \int_{\mathbb{C}} |f(z)|^2e^{-\psi(z)}\;dm(z)$, $\psi(z) = \varphi(z) + \log \kappa(z)$), $\boxed{\overline{\partial} u = f}$, $\|u\|_\varphi \le \|f\|_{\varphi + \log \kappa}$.
</div>
...
<div class="theorem">
**Theorem.** Suppose $A$ is a closed convex subset of $H$ and $x \not \in A$. Then $\exists !$ $y \in A$ such that $\|x-y\| = \inf_{z\in A} \|x-z\|$ and $L(z) := \langle z, x-y\rangle$ satisfies $\Re L(z-y) \le 0$, $z\in A$. This implies $|L(z) - L(x)| \ge |L(y) - L(x)| = \|x-y\|^2 > 0$, $z \in A$.
</div>
...
We say that $z \rightarrow B(z)$ is **meromorphic** with a pole of **finite rank** at $z = z_0$ if there exists finite rank operators $B_1, ..., B_J$ and an analytic family of open $z \rightarrow \tilde{B}(z)$, for $z$ near $z_0$ such that $B(z) = \sum_{j=1}^J \frac{B_j}{(z - z_0)^j} + \tilde{B}(z)$.

<div class="theorem">
**Theorem.** Suppose $z_0 \in \Omega$ at which $A(z_0)^{-1} : B_2 \rightarrow B_1$ exists. Then $z \mapsto A(z)^{-1}$ is a meromorphic family of open ? with poles of finite rank.
</div>

<div class="problem">
**Example.** $B = B_1 = B_2$, $S \in \mathcal{L}_c(B,B)$, $A(z) = S - z I$, $\Omega = \mathbb{C} \backslash \{0\}$.
</div>

*Proof.*

Notice $\text{ind}(A(z)) = \text{ind}(A(z_0)) = 0$. Fix $w \in  \Omega$. $\dim \ker A(w) = n(q) = n = \dim\text{coker }A(w)$. There exists $R_- : K^n \rightarrow B_2$ and $R_{+} : B_ \rightarrow K^n$ such that ?? $E_{-+}^W (z) : K^n \rightarrow K^n$

$$\begin{pmatrix} A(w) & R_{-} \\ R_+ & 0\end{pmatrix} : B_1 \oplus K^n \rightarrow B_2 \oplus K^n$$
is invertible. There exists $U(w)$ neighborhood of $w$ wsuch that we have the schur's complement thing. Now $A(z)^{-1}$ exists is equivalent to $E_{-+}^2(z)^{-}$ existing, and then $A(z)^{-1} = E^w(z) - E_{+}^w(z) E_{-+}^w(z)^{-1} E_-^w(z)$
...
<div class="theorem">
**Theorem.** Suppose $P : B_1 \rightarrow B_2$, $B_1 \subseteq B_2$, $P - zI$ is Fredholm for $z \in \Omega$ connected open ?. Assume $\exists z_0 \in \Omega$ such that $(P- z_0 I)^{-1}$ exists. Then $z \rightarrow (P-zI)^{-1}$ is meromorphic in $\Omega$ with poles of finite rank. Moreover, $\Pi_z :=\frac{1}{2\pi i} \oint_z (wI - P)^{-1} dw$ where integral is over a positive orientation circle containing only a pole, satisfies:
<center>$$\Pi_z^2 = \Pi_z,\quad \text{rank }\Pi_z < \infty$$</center>
</div>
*Proof.*

$$\Pi_z = \frac{1}{2\pi i} \int_{\gamma_j} (w - P)^{-1}\;dw$$
$$w = wI$$, 
$$\Pi_z^2 = \left(\frac{1}{2\pi i}\right)^2 \int_{\gamma_1}\int_{\gamma_2} (w_1 - P)^{-1} (w_2 - P)^{-1}\; dw_1 \; dw_2$$
$$(w_1 - P)^{-1} - (w_2 - P)^{-1} = (w_1 - P)^{-1}((w_2 - P) - (w_1 - P))(w_2 - P)^{-1}$$
$$ = (w_1 - P)^{-1}(w_2 - w_1)(w_2 - P)^{-1}$$
$$(w_1 - P)^{-1}(w_2 - P)^{-1} = \frac{1}{w_2 - w_1}((w_1 - P)^{-1} - (w_2 - P)^{-1})$$
$$\Pi_z^2 = \frac{1}{(2 \pi i)^2} \int_{\gamma_1} \int_{\gamma_2} (w_1 - P)^{-1}\frac{1}{w_2 - w_1}\;dw_1 \;dw_2\;\frac{1}{(2 \pi i)^2} \int_{\gamma_1} \int_{\gamma_2} (w_2 - P)^{-1}\frac{1}{w_2 - w_1}\;dw_1 \;dw_2$$
Integral outside is zero.

Weak Topology and Weak Star Topology and Other things
---------------------------------------------------------

- $F$, $G$ vector spaces, $F \times G \ni (x,y) \mapsto \langle x, y\rangle$ nilpotent.

Fact: If $L : F \rightarrow K$ is continuous iwth respect to $\sigma(F,G)$ where $\sigma(F,G)$ is the topology on $F$ by seminorms $x \mapsto |\langle x, y\rangle|$, $y \in G$, then $\exists y \in G$ such that $L(x) = \langle x, y\rangle$.
...
<div class="theorem">
**Theorem.** $\exists$ a positive finite measure $\frac{1}{\pi} \int \mathrm{Im} f(x+iy) \varphi(x)\;dx \rightarrow \int \varphi(x) \; d\mu(x)$ and $f(z) = \int \frac{1}{x-z}\;d\mu(x)$.
</div>